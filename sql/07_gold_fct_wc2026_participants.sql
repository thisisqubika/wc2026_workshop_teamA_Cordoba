...
-- Includes ranking-friendly comparable metrics (form, defense, WC goal diff).
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS workshop_team_a_cba.gold.fct_wc2026_participants (
  ...
  clean_sheet_pct       DOUBLE  COMMENT 'Historical clean-sheet rate (0-1).',
  recent_win_pct_last20 DOUBLE  COMMENT 'Recent form: win rate over last 20 played matches (0-1).',
  ...
  wc_goal_difference    BIGINT  COMMENT 'World Cup goal difference.'
)
USING DELTA
...
USING (
  SELECT
    ...
    s.avg_goals_for, s.avg_goals_against, s.clean_sheet_pct, s.recent_win_pct_last20,
    s.wc_appearances, s.wc_matches_played, s.wc_wins, s.wc_win_pct, s.wc_goal_difference
